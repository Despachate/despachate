...
export class VentasService {
  ...
  getpedidoXId(idPedido){
    return this.http.get(`${this._url}${idPedido}`).pipe(map((res:any)=>{
      return res;
    }), catchError((err: any)=>{
      this._toastService.showNotification('top', 'center', 'danger','Ocurrio un error al obtener')
      return throwError(err);
  ...
  insert(ventas: any){
    return this.http.post(this._url, ventas).pipe(map((res:any)=>{
      return res;
    }), catchError((err: any)=>{
  ...
  update(id: string, ventas: any){
    return this.http.put(`${this._url}/${id}`, ventas).pipe(map((res:any)=>{
      return res;
    }), catchError((err: any)=>{
