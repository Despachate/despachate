...
import { environment, URL_IMGS } from 'src/environments/environment';
...
import { CuponesDescuentosService } from '../../services/cuponesDescuentos/cupones-descuentos.service';
declare var $;
declare var paypal;
@Component({
...
export class DatosEntregaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this._usuarioService
      .getSaldo(this.datosUsuario[0]._id)
      .subscribe((res: any) => {
        console.log(
          '%c USUARIO BY EMAIL',
        ...
        this.saldo.saldo_disponible = res.saldoDisponible;
  ...
  calcularTotal() {
    ...
    if (this.saldo.usar) {
      if(this.total > 50){
        const saldoDescontable = this.total - 50;
        if (this.saldo.saldo_disponible >= saldoDescontable) {
          this.saldo.saldo_descontado = saldoDescontable;
          this.total -= saldoDescontable;
        }
        else if (this.saldo.saldo_disponible < saldoDescontable) {
            ...
            if (!stripe) {
              ...
              try {
                ...
                if (!!compra.saldo_comprado) {
                  await this._mailingService
                    .regalarSaldoAmigo(
                      !!compra.correo_saldo  ? compra.correo_saldo: this._usuarioService.usuario.email,
  ...
  ValidateDate() {
    ...
    let fsctd = new Date(this.fechaSeleccionada);
    let factl = new Date(this.getDate(new Date()));
    ...
    if (fsctd.getTime() < factl.getTime()) {
      ...
      if (fsctd.getDay() === 5) {
        ...
        //   'center',
        //   'warning',
        //   'El sabados no se cuenta con servicio de entrega'
        // );
        // Descomentar cuando borre lo anterior
        this._productoService._toastService.showNotification(
          'top',
          'center',
          'warning',
      ...
      if (new Date().getDay() === 6) {
        ...
        //       'center',
        //       'warning',
        //       'Hoy no se cuenta con servicio de entrega'
        //     );
        // descomentar cuando borre lo anterior
        ...
        if (new Date().getHours() >= 9 && new Date().getHours() < 12) {
          ...
          $('#inlineCheckboxMetodo1').prop('disabled', true);
          $('#inlineCheckboxMetodo2').prop('disabled', false);
          $('#inlineCheckboxMetodo3').prop('disabled', false);
          ...
          $('#inlineCheckboxMetodo1').prop('disabled', true);
          $('#inlineCheckboxMetodo2').prop('disabled', true);
          $('#inlineCheckboxMetodo3').prop('disabled', false);
