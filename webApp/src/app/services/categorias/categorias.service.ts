...
export class CategoriasService {
  ...
  getCatXDpto(dptoId){
    return this.http.get(`${this._url}/catxdpto/${dptoId}`).pipe(map((res:any)=>{
      return res;
    }), catchError((err: any)=>{
      this._toastService.showNotification('top', 'center', 'danger','Ocurrio un error al obtener')
      return throwError(err);
  ...
  insert(categoria: any){
    return this.http.post(this._url, categoria).pipe(map((res:any)=>{
      return res;
    }), catchError((err: any)=>{
  ...
  update(id: string, categoria: any){
    return this.http.put(`${this._url}/${id}`, categoria).pipe(map((res:any)=>{
      return res;
    }), catchError((err: any)=>{
