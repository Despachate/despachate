...
export class RecetasPasosService {
  _url: string = `${URL_SERVICIOS}recetas_paso/`;
  ...
  insert(paso: any) {
    return this.http.post(this._url, paso).pipe(
      map((res: any) => {
        return res;
      }),
      catchError((err: any) => {
  ...
  update(id: string, paso: any) {
    return this.http.put(`${this._url}${id}`, paso).pipe(
      map((res: any) => {
        return res;
      }),
      catchError((err: any) => {
