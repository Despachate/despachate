...
export class SubcategoriasService {
  ...
  getScatXCat(catId){
    return this.http.get(`${this._url}/scatxcat/${catId}`).pipe(map((res:any)=>{
      return res;
    }), catchError((err: any)=>{
      this._toastService.showNotification('top', 'center', 'danger','Ocurrio un error al obtener')
      return throwError(err);
  ...
  insert(subcategoria: any){
    return this.http.post(this._url, subcategoria).pipe(map((res:any)=>{
      return res;
    }), catchError((err: any)=>{
  ...
  update(id: string, subcategoria: any){
    return this.http.put(`${this._url}/${id}`, subcategoria).pipe(map((res:any)=>{
      return res;
    }), catchError((err: any)=>{
